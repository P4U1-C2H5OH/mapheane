...
function generatePdf(cert) {
  ...
  for (const [label, value] of details) {
    ...
    for (const [index, wrapped] of wrapText(value, 52).entries()) {
      lines.push(commandText('F2', 12, 230, y - index * 15, wrapped));
    }
    y -= Math.max(30, wrapText(value, 52).length * 15 + 10);
  }
  ...
  for (const line of statement) {
    lines.push(commandText('F2', 11, 88, y, line));
    y -= 16;
  }
  lines.push(commandText('F2', 11, 88, y - 4, '- Mapheane, Maseru'));
...
async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error('Certificate PDF rate limit error:', limitError);
    }
    ...
    return res.status(500).json({ error: 'Unable to generate certificate PDF. Please try again.' });
  }
}
