...
async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error('Newsletter rate limit error:', limitError);
    }

    const body = typeof req.body === 'string'
      ? JSON.parse(req.body || '{}')
      : req.body ?? {};
    const { email, name, source, trap } = body;
    ...
    if (RESEND_API_KEY && RESEND_AUDIENCE_ID && RESEND_AUDIENCE_ID !== 'placeholder') {
      try {
        ...
        console.error('Resend Audiences request failed:', audienceError);
      }
    }
    ...
    if (RESEND_API_KEY) {
      try {
        ...
        console.error('Resend welcome email request failed:', welcomeError);
      }
    }
